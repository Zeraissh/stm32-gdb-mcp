...
- [x] **Pillar F — data-driven device packs** — the four hardcoded device-fact tables (clock PLL profile, DMA request routing, irregular NVIC vectors, timer bus/width) are relocated out of the solvers into a pure `device_packs` registry keyed by family. A **device pack** is a validated JSON object (schema `stm32-device-pack/v1`) of verified facts for one family; **STM32F4/L4 ship as built-in packs** (facts moved verbatim, so behavior is byte-identical). `dma_solver`/`interrupt_solver`/`timer_solver`/`clock_solver` now read facts through registry accessors (public signatures unchanged). New tool **`load_device_pack`** registers a pack from `path`/inline `pack` (`allow_override` to shadow a built-in; no args → coverage report). Honest by design: a malformed pack is rejected with a problem list and never half-loaded, an unmodelled family stays `unresolved`, and covering a new family (G4/F7/…) or peripheral kind is now "supply a verified pack", never model guessing — the same data-driven discipline GPIO AF numbers already used. Tool count 77→78 (see `2026-07-01-device-packs.md`; 22 + 6 tests; suite 564 passed / 1 skipped)
- [x] **Pillar G — end-to-end design-pipeline orchestration** — a single capstone tool **`run_pipeline`** runs the whole deterministic design half in one call, re-dispatching the existing verified tools in dependency order (`import_netlist? → import_spec? → design_framework → solve_clock_tree? → solve_timer? → render_framework → synthesize_acceptance`) and returning one consolidated report. **Pure orchestration — nothing new is guessed**: the only new logic is choosing which stages to run (optional stages gated on their input; `solve_timer` gated on the post-design plan) with what args, and aggregating every stage's honest `unresolved`/`conflict` items — each tagged with its originating stage + type — into a single "human decisions / data still needed" list. Honest status: a required-stage hard error (no board, invalid input) stops with `pipeline_status=blocked` at that stage keeping the sub-tool's own code/message; expected gaps surface as `complete_with_unresolved`; a clean run is `complete`. The report hands back the products in hand (`mcu`, rendered `files`, `acceptance`), always as a `content_success`. Deterministic design half only — it stops before build/flash/verify (the Pillar C hardware loop) and hands off. This was the last of the five pipeline-review weak points (#4). Tool count 78→79, pure module `pipeline.py` (see `2026-07-01-design-pipeline.md`; 14 + 4 tests; suite 582 passed / 1 skipped)
