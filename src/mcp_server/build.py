...
def parse_keil_built_target(output: str) -> str | None:
    """Extract the target name from Keil UV4 log output.

    UV4 logs contain lines like ``Build target 'MyTarget'`` or
    ``Rebuild target 'MyTarget'``.  Returns the first match, or None.
    """
    m = re.search(r"(?:Build|Rebuild) target '([^']+)'", output or "")
