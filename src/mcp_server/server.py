...
def _dispatch_tool(name: str, arguments: dict | None) -> list[TextContent]:
    ...
    try:
        ...
        if name == "start_debug_session":
            ...
            return [content_success(
                {
                    ...
                    "port": port,
                    "symbols_loaded": symbols,
                    "server_args_source": "profile" if inferred_server_args else "arguments",
                },
                raw_response=resp,
            )]
            ...
            result = suggest_server_args(arguments["mcu"], probe, scripts_dir=scripts_dir)
            result["probe"] = probe
